...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() {
		...
		// Need to first find a list of earthquakes
		HashMap<String, Integer> numCountryQuakes = new HashMap<>();
		int numOceanQuakes = 0;
		for (Marker qm : quakeMarkers) {
			// qm could be a LandQuakeMarker or an OceanQuakeMarker
			if (qm.getClass() == LandQuakeMarker.class) {
				LandQuakeMarker lqm = (LandQuakeMarker) qm;
				String country = lqm.getCountry();

				if (!numCountryQuakes.containsKey(country)) {
					numCountryQuakes.put(country, 1);
				} else {
					int currentCount = numCountryQuakes.get(country);
					numCountryQuakes.put(country, ++currentCount);
				}
			} else {
				numOceanQuakes++;
			}
		...
		for (String country : numCountryQuakes.keySet()) {
			System.out.println(country + " : " + numCountryQuakes.get(country));
		}
		System.out.println("Ocean Quakes: " + numOceanQuakes);

	}

	private void printRadius(List<Marker> quakeMarkers) {
		for (Marker m : quakeMarkers) {
			EarthquakeMarker eqm = (EarthquakeMarker) m;
			System.out.println(eqm.getRadius());
		}
	}
