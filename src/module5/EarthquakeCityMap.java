...
public class EarthquakeCityMap extends PApplet {
	...
	public void mouseClicked() {
		...
		// Tests whether the lastClick variable refers to a null object or not
		if (lastClicked != null) {
			System.out.println("Clear selection.");

			// If not null, de-select whichever marker was clicked on last
			...
			System.out.println("Checking for marker type.");

			// Need to check if mouseX and mouseY is a marker
			for (Marker qm : quakeMarkers) {
				if (qm.isInside(map, mouseX, mouseY)) {
					lastClicked = (EarthquakeMarker) qm;
					lastSelected = (EarthquakeMarker) qm;
					System.out.println("EarthquakeMarker detected.");
					break;
				}
			}
			...
			for (Marker cm : cityMarkers) {
				if (cm.isInside(map, mouseX, mouseY)) {
					lastClicked = (CityMarker) cm;
					lastSelected = (CityMarker) cm;
					System.out.println("CityMarker detected.");
					break;
				}
			}

			// Determine which marker is being selected (if any) and hide / display other
			if (lastSelected == null) {
				System.out.println("No marker detected.");
				return;
			}

			if (lastSelected.getClass() == CityMarker.class) {
				// When a city marker is selected, all earthquakes which contain in that city in
				// their threat circle are displayed
				// Other cities and earthquakes are hidden
				System.out.println("Processing CityMarker.");

				CityMarker cm = (CityMarker) lastSelected;

				for (Marker otherCity : cityMarkers) {
					if (!otherCity.equals(cm))
						otherCity.setHidden(true);
					else
						otherCity.setHidden(false);
				}
				...
				for (Marker eq : quakeMarkers) {
					...
					if (cm.getDistanceTo(eqm.getLocation()) <= threatCircle)
						eq.setHidden(false);
					else
						eq.setHidden(true);
				}
			} else {
				// When an eq marker is selected, all cities within the threat circle are
				// displayed on the map
				// and all other cities and earthquakes are hidden
				// Use the threatCircle() method in the EarthquakeMarker class
				System.out.println("Processing EarthquakeMarker.");

				EarthquakeMarker eqm = (EarthquakeMarker) lastSelected;
				double threatCircle = eqm.threatCircle();

				for (Marker otherEq : quakeMarkers) {
					if (!otherEq.equals(eqm))
						otherEq.setHidden(true);
					else
						otherEq.setHidden(false);
				}
