...
public class EarthquakeCityMap extends PApplet {
	...
	private boolean threatCircleShown = false;
	...
	private void showAllThreatCircles() {
		for (Marker qm : quakeMarkers) {
			((EarthquakeMarker) qm).setIsShowThreatCircle(true);
		}
	}

	private void hideAllThreatCircles() {
		for (Marker qm : quakeMarkers) {
			((EarthquakeMarker) qm).setIsShowThreatCircle(false);
		}
	}
	...
	private void addShowThreatCircle() {
		// TODO Auto-generated method stub
		if (!threatCircleShown)
			fill(255);
		else
			fill(0, 120, 215);
		int x = 25;
		...
		if (!threatCircleShown)
			...
			fill(255);
		text("Show Threat Circles", x + 20, y + 12.5f);
	}
	...
	private void sortAndPrint(int numToPrint) {
		...
		// There is a method in the List interface named toArray()
		// Sort the array of earthquake markers in reverse order
		// Print out top numToPrint earthquakes
		// If numToPrint is larger, just print out all the earthquakes
		Object[] eqArray = quakeMarkers.toArray();
		Arrays.sort(eqArray);

		for (int i = 0; i < numToPrint; i++) {
			if (i == eqArray.length - 1)
				break;
	...
	public void mouseClicked() {
		...
		checkIfShowThreatCircle();
	}

	private void checkIfShowThreatCircle() {
		// TODO Auto-generated method stub
		// int x = 25;
		// int y = 620;
		// rect(x, y, 150, 30);
		if (mouseX > 25 && mouseX < 175 && mouseY > 620 && mouseY < 650) {
			if (!threatCircleShown) {
				showAllThreatCircles();
				threatCircleShown = true;
			} else {
				hideAllThreatCircles();
				threatCircleShown = false;
			}
		}
	}
